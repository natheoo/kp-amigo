...
class SyncController extends AppController{
	...
	public function sync_dump(){
		if (...) {
			...
			// $this->set('last_sync',$last_sync);

			/* 2. get list of operations happened after last synchronization from mddb.mddb */
			$changes = array();
			if (!empty($last_sync)) {
				// $changes = $this->Mddb->find('all',array(
				// 	'conditions' => array('timestamp_op >' => $last_sync['Waktusyncakhir']['timestamp_sync']),
				// 	'fields' => array('jenis_op','nama_tabel','prim_key')
				// 	)
				// );
				...
				// $query_changes = "SELECT Mddb.*, m.* FROM mddb Mddb LEFT JOIN mddb m ";
				// $query_changes .= "ON Mddb.nama_tabel = m.nama_tabel AND Mddb.prim_key = m.prim_key ";
				// $query_changes .= "WHERE Mddb.timestamp_op > '$last_sync'";
				$changes = $this->Mddb->query($query_changes);
			}
			$this->set('changes',$changes);
			// $log = $this->Mddb->getDataSource()->getLog(false,false);
			// debug($log);
			...
			for ($i=0; $i < sizeof($changes); $i++) { 
				...
				foreach ($record[$ctrl] as $field => $value) {
					...
					// $this->set('col_type',$col_type[0]['COLUMNS']['Type']);
				...
				$this->set('str_fields',$str_fields);
				$this->set('str_values',$str_values);
			...
			$file_name = "D:\bimbo_$dumpDate.sql";
